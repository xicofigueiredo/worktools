    ...
    rows.each_with_index do |row, i|
      ...
      begin
        ...
        if email_val.blank?
          ...
          puts "[WARN] Line #{lineno}: #{reason} | user_email: #{email_val} | department_name: #{dept_val} | raw_row: #{row_h.to_json}"
          failed += 1
          next
        end
        ...
        if dept_val.blank?
          ...
          puts "[WARN] Line #{lineno}: #{reason} for email #{email_val} | user_email: #{email_val} | department_name: #{dept_val} | raw_row: #{row_h.to_json}"
          failed += 1
          next
        end
        ...
        unless user
          ...
          puts "[WARN] Line #{lineno}: #{reason} (#{email_lookup}) | user_email: #{email_lookup} | department_name: #{dept_val} | raw_row: #{row_h.to_json}"
          failed += 1
          next
        end
        ...
        unless dept
          ...
          puts "[WARN] Line #{lineno}: #{reason} ('#{dept_val}') | user_email: #{email_lookup} | department_name: #{dept_val} | raw_row: #{row_h.to_json}"
          failed += 1
          next
        end
        ...
        begin
          ...
          if ud.persisted?
            created += 1
            puts "[INFO] Line #{lineno}: created UsersDepartment user_id=#{user.id} department_id=#{dept.id}"
          else
            skipped_existing += 1
            puts "[INFO] Line #{lineno}: users_department already exists for user_id=#{user.id}, department_id=#{dept.id}"
          end
        rescue ActiveRecord::RecordNotUnique
          # Unique index race or duplicate -> treat as skipped
          skipped_existing += 1
          puts "[INFO] Line #{lineno}: users_department already exists (unique conflict) user_id=#{user.id} dept_id=#{dept.id}"
        end

      rescue => e
        puts "[ERROR] Line #{lineno}: unexpected error: #{e.class}: #{e.message}\n#{e.backtrace.first(6).join("\n")} | user_email: #{email_val rescue ''} | department_name: #{dept_val rescue ''} | raw_row: #{(row_h rescue {}).to_json}"
        failed += 1
        next
      end
    end

    puts "Import finished. processed=#{processed} created=#{created} skipped_existing=#{skipped_existing} failed=#{failed}"
  end
end
