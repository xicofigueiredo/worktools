class LwsTimelinesController < ApplicationController
  ...
  def update
    ActiveRecord::Base.transaction do
      if @lws_timeline.update(lws_timeline_params)
        user_topics_data = save_user_topics_data(@lws_timeline)  # Assuming this method returns the needed data
        recreate_timelines_for_subjects(@lws_timeline, user_topics_data)
        redirect_to lws_timelines_path, notice: 'LWS Timeline and associated timelines were successfully updated.'
      else
        render :edit
      end
    end
  ...
  def create_timelines_for_subjects(lws_timeline, user_topics_data = {})
  ...
  def recreate_timelines_for_subjects(lws_timeline, user_topics_data)
